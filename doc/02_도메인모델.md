...
|     uuid      |    BIGINT    |      NOT NULL, UK       |  외부 식별자 (Snowflake 기반, 제휴사/API 용도)   |
|     email     | VARCHAR(100) |      NOT NULL, UK       |               로그인용 이메일               |
|   password    | VARCHAR(255) |        NOT NULL         |              암호화된 비밀번호               |
|     name      | VARCHAR(50)  |        NOT NULL         |                사용자 이름                |
| member_status |     ENUM     |    DEFAULT 'ACTIVE'     | 회원 상태 (ACTIVE, WITHDRAWN, SUSPENDED) |
|  login_type   |     ENUM     |     DEFAULT 'LOCAL'     | 로그인 유형 (LOCAL, KAKAO, GOOGLE, NAVER) |
|     grade     |     ENUM     |     DEFAULT 'BASIC'     |   ENUM(‘BASIC’, ‘VIP’, ‘EMPLOYEE’)   |
|    channel    | VARCHAR(50)  |        NULLABLE         |               유입 채널 정보               |
|  blacklisted  |   BOOLEAN    | NOT NULL, DEFAULT FALSE |               블랙리스트 여부               |
...
| event_status |     ENUM     |       DEFAULT 'READY'       | 이벤트 상태(READY, IN_PROGRESS, FINISHED) |
|  created_at  |   DATETIME   |          NOT NULL           |                생성 시각                 |
|  created_by  | VARCHAR(50)  |          NOT NULL           |                 생성자                  |
|  updated_at  |   DATETIME   |          NOT NULL           |             수정 시각              |
|  updated_by  | VARCHAR(50)  |          NULLABLE           |                 수정자                  |
...
|    **필드명**     |   **타입**    |          **제약조건**           |              **설명**              |
|:--------------:|:-----------:|:---------------------------:|:--------------------------------:|
|    batch_id    |   BIGINT    | AUTO_INCREMENT, PRIMARY KEY |          발급 회차의 고유 식별자           |
|  inventory_id  |   BIGINT    |        NOT NULL, FK         |    해당 발급 회차가 속한 쿠폰 발급 풀의 식별자     |
|  issue_count   |     INT     |          NOT NULL           |              쿠폰발급개수              |
|   issue_type   |    ENUM     |       DEFAULT 'SELF'        |  자체발급 / 제휴매핑 발급 구분(SELF,MAPPED)  |
| valid_from_at  |  DATETIME   |          NOT NULL           |          쿠폰 사용 가능 시작 시간          |
| valid_until_at |  DATETIME   |          NOT NULL           |             쿠폰 만료 시간             |
|  batch_status  |    ENUM     |       DEFAULT 'READY'       | 이벤트 상태(PENDING, READY, DISABLED) |
|   created_at   |  DATETIME   |          NOT NULL           |              생성 시각               |
|   created_by   | VARCHAR(50) |          NOT NULL           |               생성자                |
|   updated_at   |  DATETIME   |          NOT NULL           |           수정 시각            |
|   updated_by   | VARCHAR(50) |          NULLABLE           |               수정자                |
...
|              code_status               |    ENUM     |      DEFAULT 'AVAILABLE'       | 쿠폰 상태 (AVAILABLE, ISSUED, USED, EXPIRED) |
...
|         upload_path          | VARCHAR(100) |          NULLABLE           |           파일이 저장 경로           |
|         uploaded_at          |   DATETIME   |          NOT NULL           |         쿠폰 코드 업로드 시각          |
|         uploaded_by          | VARCHAR(50)  |          NOT NULL           |          파일 업로드 사용자           |
|          created_at          |   DATETIME   |          NOT NULL           |             생성 시각             |
|          created_by          | VARCHAR(50)  |          NOT NULL           |              생성자              |
|          updated_at          |   DATETIME   |          NOT NULL           |          수정 시각          |
|          updated_by          | VARCHAR(50)  |          NULLABLE           |              수정자              |
...
|              mapping_status              |     ENUM     |               DEFAULT 'MAPPED'               | 매핑 상태 (MAPPED, FAILED) |
|                created_at                |   DATETIME   |                   NOT NUL                    |         생성 시각          |
|                created_by                | VARCHAR(50)  |                   NOT NULL                   |          생성자           |
|                updated_at                |   DATETIME   |                   NOT NULL                   |      수정 시각       |
|                updated_by                | VARCHAR(50)  |                   NULLABLE                   |          수정자           |
...
|            coupon_id            |   BIGINT    |         NOT NULL, FK          |           내부용 쿠폰 식별자           |
|           issue_type            |    ENUM     |        DEFAULT 'SELF'         | 자체발급 / 제휴매핑 발급 구분(SELF,MAPPED) |
|            issued_at            |  DATETIME   |           NOT NULL            |             쿠폰발급시각             |
|           is_canceled           |   BOOLEAN   |         DEFAULT FALSE         |          쿠폰 발급 취소 여부           |
|           canceled_at           |  DATETIME   |           NULLABLE            |              취소시각              |
|           created_at            |  DATETIME   |           NOT NULL            |             생성 시각              |
|           created_by            | VARCHAR(50) |           NOT NULL            |              생성자               |
|           updated_at            |  DATETIME   |           NOT NULL            |          수정 시각           |
|           updated_by            | VARCHAR(50) |           NULLABLE            |              수정자               |
...
|            **필드명**            |   **타입**    |           **제약조건**            |            **설명**            |
|:-----------------------------:|:-----------:|:-----------------------------:|:----------------------------:|
|           usage_id            |   BIGINT    |  AUTO_INCREMENT, PRIMARY KEY  |         발급 이력 고유 식별자         |
|           member_id           |   BIGINT    |         NOT NULL, FK          |          회원 고유 식별자           |
|           coupon_id           |   BIGINT    |         NOT NULL, FK          |          내부용 쿠폰 식별자          |
|            used_at            |  DATETIME   |           NOT NULL            |           쿠폰 사용시간            |
|         used_channel          | VARCHAR(50) |         DEFAULT 'WEB'         | 쿠폰 사용 채널 (APP, WEB, OFFLINE) |
|          created_at           |  DATETIME   |           NOT NULL            |            생성 시각             |
|          created_by           | VARCHAR(50) |           NOT NULL            |             생성자              |
|          updated_at           |  DATETIME   |           NOT NULL            |         수정 시각          |
|          updated_by           | VARCHAR(50) |           NULLABLE            |             수정자              |
...
|          event_id          |   BIGINT    |         NOT NULL, FK         |  참여한 이벤트 고유 식별자  |
|         member_id          |   BIGINT    |         NOT NULL, FK         | 이벤트 참여 회원 고유 식별자 |
|      participated_at       |  DATETIME   |           NOT NUL            |     이벤트참여시각      |
|         created_at         |  DATETIME   |           NOT NULL           |      생성 시각       |
|         created_by         | VARCHAR(50) |           NOT NULL           |       생성자        |
|         updated_at         |  DATETIME   |           NOT NULL           |   수정 시각    |
|         updated_by         | VARCHAR(50) |           NULLABLE           |       수정자        |
...
### 12) DRAW_WINNER(당첨자 정보 (추첨 결과))

|          **필드명**           |   **타입**    |           **제약조건**           |          **설명**          |
|:--------------------------:|:-----------:|:----------------------------:|:------------------------:|
|         winner_id          |   BIGINT    | AUTO_INCREMENT, PRIMARY KEY  |      이벤트 당첨자 고유 식별자      |
|          event_id          |   BIGINT    |         NOT NULL, FK         |      참여한 이벤트 고유 식별자      |
|         member_id          |   BIGINT    |         NOT NULL, FK         |     이벤트 참여 회원 고유 식별자     |
|         prize_type         |    ENUM     |       DEFAULT 'POINT'        | 당첨된 상품 타입(POINT, COFFEE) |
|          drawn_at          |  DATETIME   |           NOT NULL           |         이벤트참여시각          |
|         created_at         |  DATETIME   |           NOT NULL           |          생성 시각           |
|         created_by         | VARCHAR(50) |           NOT NULL           |           생성자            |
|         updated_at         |  DATETIME   |           NOT NULL           |       수정 시각        |
|         updated_by         | VARCHAR(50) |           NULLABLE           |           수정자            |
...
### 13) COUPON_POLICY(쿠폰정책)

|     **필드명**      |         **타입**         |          **제약조건**           |        **설명**        |
|:----------------:|:----------------------:|:---------------------------:|:--------------------:|
|    policy_id     |         BIGINT         | AUTO_INCREMENT, PRIMARY KEY |    이벤트 당첨자 고유 식별자    |
|   policy_type    |          ENUM          |       DEFAULT 'FIXED'       | 할인유형(FIXED, PERCENT) |
| discount_amount  |     DECIMAL(10,2)      |          NULLABLE           |   할인 금액(고정 할인시 사용)   |
| discount_percent |     DECIMAL(10,2)      |          NULLABLE           |  할인 비율(퍼센트 할인시 사용)   |
| min_order_amount |     DECIMAL(10,2)      |          NULLABLE           |    적용가능한 최소 주문금액     |
| min_order_count  |          INT           |          NULLABLE           |    적용가능한 최소 주문갯수     |
|    created_at    |        DATETIME        |          NOT NULL           |        생성 시각         |
|    created_by    |      VARCHAR(50)       |          NOT NULL           |         생성자          |
|    updated_at    |        DATETIME        |          NOT NULL           |     수정 시각      |
|    updated_by    |      VARCHAR(50)       |          NULLABLE           |         수정자          |
...
## 4. 관계 다이어그램 (UML 형태)
![image](./etc/coupon-2025-06-16T18_05_02.png)
